...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  const { metadata, chainId, finalTokenId } = req.query

  const attribtues = await getMetadataAttributes({
    metadata: metadata as Address,
    chainId: parseInt(chainId as string),
    finalTokenId: parseInt(finalTokenId as string),
  })

  res.status(200).send(attribtues)
}

const getMetadataAttributes = async ({
  metadata,
  finalTokenId,
  chainId,
}: {
