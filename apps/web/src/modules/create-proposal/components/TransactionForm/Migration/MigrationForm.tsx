...
import { defaultInputLabelStyle } from './Migration.css'
import migrationFormSchema, { MigrationFormValues } from './Migration.schema'

export interface MigrationFormProps {
  onSubmit?: (
    values: MigrationFormValues,
    actions: FormikHelpers<MigrationFormValues>
  ) => void
  disabled?: boolean
  checkpoint: 0 | 1 | 2 | 3
}
...
export const MigrationForm: React.FC<MigrationFormProps> = ({
  ...
  const initialValues: MigrationFormValues = {
    name: '',
    symbol: '',
    description: '',
    mediaUrl: '',
  ...
  const handleSubmit = useCallback(
    (values: MigrationFormValues, actions: FormikHelpers<MigrationFormValues>) => {
  ...
  return (
    <Box w="100%">
      {checkpoint > 0 && <div>Checkpoint 1 is completed</div>}
      {checkpoint > 1 && <div>Checkpoint 2 is completed</div>}
      {checkpoint > 2 && <div>Checkpoint 3 is completed</div>}
      <Formik
        initialValues={initialValues}
        validationSchema={migrationFormSchema}
